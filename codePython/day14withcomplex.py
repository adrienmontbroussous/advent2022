...
def moveSand(rockList,xSand,ySand,ymaxRock,xMinRock,xMaxRock):
    ...
    if ySand>=ymaxRock or xMinRock>xSand or xMaxRock<xSand:
        stop=True
    elif (complex(xSand,ySand+1)) not in rockList:
        #on descend
        newXSand=xSand
        nexYsand=ySand+1
    elif (complex(xSand-1,ySand+1)) not in rockList:
        ...
        newRockList.append(complex(xSand,ySand))
...
def moveSand2(rockList,xSand,ySand,ymaxRock,xMinRock,xMaxRock):
    ...
    if (complex(newXSand,nexYsand)) in rockList:
